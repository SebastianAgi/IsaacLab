...
class FrankaGran2D3DOF(DirectRLEnv):
    ...
    def _compute_rewards(
        ...
        norm_goal_dist = torch.where(
            moving_wrong_direction & ~in_target,  # Wrong direction and not in target
        ...
        # # Bonus for objects in target area that aren't moving (stable position)
        # stable_target_reward = (~is_moving & in_target).sum(dim=-1) / self.cfg.num_grans
        
        # Total reward combining all components
        # rewards = norm_reward + (stable_target_reward) + direction_penalty_norm
        rewards = norm_reward + direction_penalty_norm

        # print(
        #       f"\nnorm_goal_dist_sum: {norm_reward[0]}"
        #       f"\ndirection_penalty_sum: {direction_penalty_norm[0]}"
        #     #   f"\nstable_target_reward: {stable_target_reward[0]}"
        #       f"\nrewards: {rewards[0]}"
        #       f"\n---"
        #     )
