...
class FrankaGran2D3DOFNewReset(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaGranCfg, render_mode: str | None = None, **kwargs):
        ...
        self.start_joint_pos = torch.tensor([ 0.2794,  0.4953,  0.2504, -2.2270, -0.2744,  2.6936, -0.1548], dtype=torch.float, device=self.device)
        self.start_joint_vel = torch.zeros_like(self.start_joint_pos)
    ...
    def _apply_action(self):
        ...
        ## Reset arm position to start pose every 100 steps
        # periodic_reset_mask = (self.episode_length_buf % 480 == 0)
        # if periodic_reset_mask.any():
        #     joint_pos_des[periodic_reset_mask] = self.start_joint_pos
        #     reset_env_ids = torch.nonzero(periodic_reset_mask, as_tuple=False).squeeze(-1)
        #     self._robot.set_joint_position_target(self.start_joint_pos, env_ids=reset_env_ids)
        #     self._robot.write_joint_state_to_sim(self.start_joint_pos, self.start_joint_vel, env_ids=reset_env_ids)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        self.reached_goal_sum += terminated2.sum()
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._robot.set_joint_position_target(self.start_joint_pos, env_ids=env_ids)
        self._robot.write_joint_state_to_sim(self.start_joint_pos, self.start_joint_vel, env_ids=env_ids)
        ...
        # success_threshold_distance = 10000
        # distance_increment = 0.001
        # distance_increment_steps = torch.div(self.goals_per_episode, success_threshold_distance, rounding_mode='floor')
        # distance_increase = -distance_increment_steps * distance_increment + self.cfg.target_pose[1]

        # distance_increase = torch.clamp(distance_increase, self.cfg.target_area[1][1] + self.cfg.goal_radius, 0.0)
        ...
        self.target_pos[env_ids,1] = self.cfg.target_pose[1] - 0.2 # keep y position constant
        ...
        # success_threshold_spawn_radius = 1000
        # radius_increment = self.cfg.spawn_area_radius
        # increment_steps = torch.div(self.goals_per_episode, success_threshold_spawn_radius, rounding_mode='floor')
    ...
    #     pos = self.objects_pos_mean[env_ids] - self.scene.env_origins[env_ids] + (1.2*self.cfg.spawn_area_radius) * v_hat
    ...
    def _get_observations(self):
        ...
        ## Set objecst in the target area to 0
        # Create a mask where the norm is less than 0.1 - but maintain proper dimensions
        mask = (torch.norm(objects_pos_rel, p=2, dim=-1) < 0.1).unsqueeze(-1).expand_as(objects_pos_rel)

        # Zero out only the x,y pairs that have a norm less than 0.1
        objects_pos_rel = torch.where(mask, torch.zeros_like(objects_pos_rel), objects_pos_rel)
