...
from isaaclab.utils.math import quat_mul, sample_uniform, subtract_frame_transforms, matrix_from_quat
from .franka_gran_cfg import FrankaGranCfg


class FrankaGranYaw(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaGranCfg, render_mode: str | None = None, **kwargs):
        ...
        self.robot_dof_tagets = torch.zeros((self.num_envs, self._robot.num_joints), device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.yaw = self.robot_dof_targets[:, -1] + self.robot_dof_speed_scales * self.dt * self.actions[:, -1] * self.cfg.action_scale
        # yaw[:] = torch.clamp(yaw, self.robot_dof_lower_limits, self.robot_dof_upper_limits)
    ...
    def _apply_action(self):
        ...
        self.robot_dof_targets = self.diff_ik_controller.compute(self.ee_position, self.ee_orientation_controller, jacobian, joint_pos)
        
        self.robot_dof_targets[:,-1] = self.yaw
