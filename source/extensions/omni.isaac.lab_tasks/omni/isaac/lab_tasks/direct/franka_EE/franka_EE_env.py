...
class FrankaEndEffectorEnv(DirectRLEnv):
    ...
    cfg: FrankaEndEffectorEnvCfg
    
    def __init__(self, cfg: FrankaEndEffectorEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        spawn_pose_tensor = torch.tensor(cfg.spawn_pose, dtype=torch.float, device=self.device)
        self.object_pos[:,:] = spawn_pose_tensor
        ...
        self.ee_orientation[:,:] = hand_pose[3:7]
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        # Actions are in the order [dx, dy, dz, qw, qx, qy, qz]

        # Clamp and scale the actions
        self.actions = actions.clone().clamp(-1.0, 1.0)
        
        # normalize the quaternion in actions
        quat_normalized = torch.nn.functional.normalize(self.actions[:,3:], p=2.0, dim=-1, eps=1e-12)

        delta_position = self.actions[:, :3] * self.cfg.action_scale * self.dt


        delta_orientation = quat_normalized * self.cfg.action_scale * self.dt

        ######################################################################
        # Limit the ee_position to never go outside the workspace boundaries #
        ######################################################################
        # Update the end effector position and orientation
        self.ee_position += delta_position
        self.ee_position[:, 0] = torch.clamp(self.ee_position[:, 0], -0.5, 0.5) 
    ...
    def _apply_action(self):
        ...
        #################################################################################
        # VISUALIZING HERE MIGHT INTRODUCE LAG AS IT IS CALLED BEFORE EVERY STEP UPDATE #
        #################################################################################
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # self.spawn_distance = self.target_pos - self.object_pos
    ...
    def _get_observations(self):
        ...
        # OBSERVATIONS:
        # 1.1 Joint states (7 joints)
        # 1.2 Joint velocities (7 joints)
        # 2. End-effector pose (position + orientation)
        # 3. Coordinate of the goal position
        # 4. Coordinate of the object to be moved
        # 5. Action taken by the agent
        
        # Joint positions scaled to [-1, 1], shape (num_envs, 7)
        dof_pos_scaled = (
            2.0
        ...
        object_to_target_dists = (self.target_pos + self.scene.env_origins) - self.object_pos
        ...
        observations = torch.cat(
            (
                ...
                # dof_pos_scaled, # 7 joints
                # self._robot.data.joint_vel * self.cfg.dof_velocity_scale, # 7 joints
                # self.actions,         # [dx, dy, dz, qw, qx, qy, qz]
            ),
            dim=-1,
        )
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        # data for object
        self.object_pos = self._object.data.root_pos_w # - self.scene.env_origins (want it in global frame)
    ...
    def _compute_rewards(
        ...
        # finegrained_object_pusher_dist_reward = 1 - torch.tanh((object_pusher_dist_reward) / 0.1)

        # Tanh normalized distance reward
        # d_target = torch.norm(target_pos - object_pos, p=2, dim=-1)
        # target_dist_reward = 1 - torch.tanh((d_target) / 0.1)
        goal_dist = torch.norm(object_pos - target_pos, p=2, dim=-1)#.unsqueeze(-1)
        norm_goal_dist = torch.div(goal_dist, spawn_distance)#.squeeze(-1)# goal_dist / spawn_distance
        reward_dist = 1 - torch.tanh(3*norm_goal_dist)
        # rew_dist = 2 * torch.exp(-target_reward_scale * goal_dist)

        # ## Alignment reward
        # gripper_forward_axis_world = tf_vector(robot_grasp_rot, hand_forward_axis)  # Compute alignment (cosine similarity)
        # target_direction = torch.tensor([0, 0, -1], device=self.device)             # Define the target direction (negative z-axis)
        # dot = torch.sum(gripper_forward_axis_world * target_direction, dim=-1)      # Compute the cosine similarity (dot product)
        ...
        rewards = (
            ...
            # + dist_reward_scale * finegrained_object_pusher_dist_reward
            + target_reward_scale * reward_dist
