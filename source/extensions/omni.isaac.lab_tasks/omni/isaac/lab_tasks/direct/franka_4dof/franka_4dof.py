...
class Franka4dof(DirectRLEnv):
    ...
    def __init__(self, cfg: Franka4dofCfg, render_mode: str | None = None, **kwargs):
        ...
        self.yaw_difference = torch.zeros((self.num_envs), dtype=torch.float, device=self.device)
        self.delta_yaw_quat = torch.zeros((self.num_envs, 4), device=self.device)
        self.clamped_yaw_quat = torch.zeros((self.num_envs, 4), device=self.device)
        ...
        self.ee_orientation[:,:] = new_quat
        self.init_ee_orientation = self.ee_orientation.clone()

        print(f"Initial EE orientation: {self.init_ee_orientation}")
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        # Actions are in the order [dx, dy, dz, d_yaw_rad]
        ...
        clamped_yaw = torch.clamp(delta_yaw, -2.8973 + 0.785398 - current_yaw, 2.8973 + 0.785398 - current_yaw)
    ...
    def _get_observations(self):
        ...
        # OBSERVATIONS:
        # 1. Pusher tip position
        # 2. yaw of end-effector
        # 3. Object position
        # 4. Target position

        observations = torch.cat(
            (
                self.robot_grasp_pos,                 # [x, y, z]         world frame
                self.ee_orientation,                  # [qw, qx, qy, qz]  orientation of the end-effector
                self.object_pos,                      # [x, y, z]         world frame
                self.target_pos,                      # [x, y, z]         world frame
            ),
            dim=-1,
        )
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        # data for object
        self.object_pos = self._object.data.root_pos_w[env_ids]
    ...
    def _compute_rewards(
        ...
        d = torch.norm(object_pos - robot_grasp_pos, p=2, dim=-1) # Tanh normalized distance reward

        ## Cabinet distance reward
        dist_reward = 1.0 / (1.0 + d**2)
        dist_reward *= dist_reward
        object_pusher_dist_reward = torch.where(d <= 0.1, dist_reward * 2, dist_reward)
        ...
        # Set values below 0.01 to 0.0 to avoid large negative rewards
        norm_goal_dist = torch.where(norm_goal_dist < 0.01, torch.tensor(0.0, device=self.device), norm_goal_dist)
        ...
        # print(
        #     f'---\n'
        #     f'pusher_object : {(dist_reward_scale * d[0]).item():.4f},\n'
        #     f'norm_goal_dist: {(target_reward_scale * norm_goal_dist[0]).item():.4f},\n'
        #     f'rewards       : {rewards[0].item():.4f}'
        # )
